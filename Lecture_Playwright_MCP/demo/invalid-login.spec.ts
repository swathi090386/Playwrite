...
test.describe('VWO Login - Invalid Login Scenarios', () => {
  ...
  test('TC-LOGIN-INV-001: Login with Arabic characters in email should fail', async ({ page }) => {
    ...
    await expect(emailInput).toHaveValue('اختبار@example.com');
  ...
  test('TC-LOGIN-INV-002: Login with Chinese characters in email should fail', async ({ page }) => {
    ...
    expect(emailValue).toContain('测试');
  ...
  test('TC-LOGIN-INV-003: Login with SQL injection attempt should be safely rejected', async ({ page }) => {
    ...
    await expect(emailInput).toHaveValue(sqlInjectionEmail);
    await expect(passwordInput).toHaveValue(sqlInjectionPassword);
    ...
    if (isErrorDisplayed) {
      ...
      expect(exposesDatabase).toBe(false);
    ...
    await expect(emailInputAfter).toBeEnabled();
    
    console.log('SQL injection attempt safely rejected - Test PASSED');
  });

  // TC-LOGIN-INV-004: Invalid Login with Dummy/Fake Credentials
  test('TC-LOGIN-INV-004: Login with dummy/fake credentials should fail with error', async ({ page }) => {
  ...
  test('TC-LOGIN-INV-005: Login with numeric email and special characters password should fail', async ({ page }) => {
    ...
    const emailValue = await emailInput.inputValue();
    expect(emailValue).toBe(numericEmail);
  ...
  test('VALIDATE: No sensitive information exposed in error messages', async ({ page }) => {
